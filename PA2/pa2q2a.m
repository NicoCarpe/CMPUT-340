...
% curves almost exactly overlap. This is confirmed when we observe the 
% residual values. We can see that both the residuals of the interpolated
% and theoretical data are small when comparing to the observed data.
% Notably the difference between the residual mse of both the interpolated
% and theoretical curves is very small. From this behaviour, it can be 
% said that the observations (D) do follow the laws of physics 
% (the theoretical values D_theory) and that our model was a good fit.
...
% calculate errors between the theoretical data and the original data
% residual error
y_interp_origin = polyval(poly_interp, X);
...
disp('theoretical residuals:')
disp(theory_residuals)
disp('theoretical relative residuals:')
disp(theory_rel_residuals)
disp(['theoretical residual MSE: ', num2str(theory_residual_MSE)])

disp(['residual MSE difference: ', num2str(residual_MSE_diff)])
